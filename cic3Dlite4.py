...
for k in range(0,3):
	for i in range(0,100):
		for j in range(0,100):	
			if(i%4==0 and j%4==0):
				bum=(grid[i,j,k]+grid[i+1,j,k]+grid[i+2,j,k]+grid[i+3,j,k]+grid[i,j+1,k]+grid[i+1,j+1,k]+grid[i+2,j+1,k]+grid[i+3,j+1,k]+grid[i,j+2,k]+grid[i+1,j+2,k]+grid[i+2,j+2,k]+grid[i+3,j+2,k]+grid[i,j+3,k]+grid[i+1,j+3,k]+grid[i+2,j+3,k]+grid[i+3,j+3,k])/(max*16)
				if(bum>0.8):
					c=(bum,0,0)
				elif(bum>0.5):
					c=(1,1-(bum+0.10),0)
